...
int cover(Set *members, Set *subsets, Set *covering) {
    ...
    /* Continue while there are uncovered members and candidate subsets */
    while (set_size(members) > 0 && set_size(subsets) > 0) {
        ...
        /* Covering not possible if no intersection */
        if (max_size == 0)
            return 1;
        ...
        /* Remove the subset from the set of candidate members */
