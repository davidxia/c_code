...
#include "bit.h"
#include "../ch9/bitree.h"
#include "compress.h"
#include "../ch10/pqueue.h"


static int compare_freq(const void *tree1, const void *tree2) {
    HuffNode *root1, *root2;

    /* Compare frequencies stored in root nodes of two binary trees */
...
static void destroy_tree(void *tree) {
    /* Destroy and free one binary tree from priority queue of trees */
    bitree_destroy(tree);
    free(tree);
    return;
}
...
static int build_tree(int *freqs, BiTree **tree) {
    ...
    /* Initialize priority queue of binary trees */
    *tree = NULL;
    pqueue_init(&pqueue, compare_freq, destroy_tree);

    for (c = 0; c <= UCHAR_MAX; c++) {
        if (freqs[c] != 0) {
            /* Set up binary tree for current symbol and its frequency */
            ...
            /* Insert binary tree into priority queue */
    ...
    /* Build a Huffman tree by merging trees in priority queue */
    size = pqueue_size(&pqueue);
    for (c = 1; c <= size - 1; c++) {
        /* Allocate storage for next merged tree */
        ...
        /* Allocate storage for data in root node of merged tree */
        if ((data = (HuffNode *) malloc(sizeof(HuffNode))) == NULL) {
            pqueue_destroy(&pqueue);
            free(merge);
            return -1;
        ...
        /* Insert merged tree into priority queue and free others */
    ...
    /* The last tree in priority queue is Huffman tree */
...
static void build_table(BiTreeNode *node, unsigned short code, unsigned char size, HuffCode *table) {
    if (!bitree_is_eob(node)) {
        if (!bitree_is_eob(bitree_left(node))) {
            /* Move to the left and append 0 to current code */
            build_table(bitree_left(node), code << 1, size + 1, table);
        }
        if (!bitree_is_eob(bitree_right(node))) {
            /* Move to the right and append 1 to current code */
        ...
        if (bitree_is_eob(bitree_left(node)) && bitree_is_eob(bitree_right(node))) {
            /* Ensure that current code is in big-endian format */
            code = htons(code);
...
int huffman_compress(const unsigned char *original, unsigned char **compressed, int size) {
    ...
    *compressed = NULL;
    ...
    while (ipos < size) {
        /* Get next symbol in original data */
        c = original[ipos];

        /* Write code for the symbol to the buffer of compressed data */
...
int huffman_uncompress(const unsigned char *compressed, unsigned char **original) {
    ...
    /* Initially there's no buffer of original data */
    *original = orig = NULL;

    /* Get the header info from buffer of compressed data */
    hsize = sizeof(int) + (UCHAR_MAX + 1);
    memcpy(&size, compressed, sizeof(int));
    for (c = 0; c <= UCHAR_MAX; c++)
        freqs[c] = compressed[sizeof(int) + c];

    /* Rebuild Huffman tree used previously to compress data */
    if (build_tree(freqs, &tree) != 0)
        return -1;
    ...
    while (opos < size) {
        ...
        /* Write symbol in leaf node to buffer of original data */
        if (bitree_is_eob(bitree_left(node)) && bitree_is_eob(bitree_right(node))) {
            if (opos > 0) {
