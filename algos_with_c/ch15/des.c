...
/* Define number of rotations for computing subkeys */
static const int DesRotations[16] = {
    1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1
};

/* Define mapping for permuted choice for subkeys */
...
/* Define a mapping for initial permutation of data blocks */
...
/* Define a mapping for expansion permutation of data blocks */
...
/* Define tables for S-box substitutions performed for data blocks */
...
/* Define a mapping for P-box permutation of data blocks */
static const int DesPbox[32] = {
    16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10,
    2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25
};

/* Define a mapping for final permutation of data blocks */
...
static int des_main(const unsigned char *source, unsigned char *target,
    ...
    /* If key is NULL, use subkeys as computed in previous call */
    if (key != NULL) {
    ...
    /* Make local copy of source text */
    memcpy(temp, source, 8);
    /* Do initial permutation */
    permute(temp, DesInitial, 64);
    /* Split source text into left and right block of 32 bits */
    memcpy(lblk, &temp[0], 4);
    memcpy(rblk, &temp[4], 4);

    /* Encipher or decipher source text */
    for (i = 0; i < 16; i++) {

        /* Begin computation of f */
        memcpy(fblk, rblk, 4);
        /* Permute and expand copy of right block into 48 bits */
        permute(fblk, DesExpansion, 48);

        /* Apply appropriate subkey for the round */
        ...
        for (j = 0; j < 8; j++) {
            ...
            /* Do S-box substitution for current six-bit block */
        ...
        /* Compute the XOR of left block and f */
        bit_xor(lblk, fblk, xblk, 32);
        /* Set left block for the round */
        memcpy(lblk, rblk, 4);
        /* Set right block for the round */
        memcpy(rblk, xblk, 4);
    }

    /* Set target text to the rejoined right and left blocks */
