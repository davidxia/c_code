...
int main() {
    ...
    /* is restarted before the before the required wait time */
    /* expires. */
    ...
    if ((sd2 = accept(sd, (struct sockaddr *) &their_addr, &sin_zie)) < 0) {
    ...
    printf("Got connection from the f**king client: %s\n", inet_ntoa(their_addr.sin_addr));
    ...
    /* shows the data */
    printf("Received data from the f**king client: %s\n", buffer);

    /* Echo some bytes of string, back */
    /* to the client by using the write() function. */
